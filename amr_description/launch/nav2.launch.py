...
def generate_launch_description():
    ...
    nav2_launch_cmd = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            'autostart': autostart,
            'map': os.path.join(pkg_amr, 'config', 'coe_final.yaml'),
            'params_file': os.path.join(pkg_amr, 'config', 'nav2_params.yaml'),
            'package_path': pkg_amr, 
        }.items()
    )
    ...
    amcl_node = Node(
        ...
        name='amcl',
        output='screen',
        parameters=[os.path.join(pkg_amr, 'config', 'amcl_params.yaml')],
    )
    ...
    map_server_node = Node(
        ...
        name='map_server',
        output='screen',
        parameters=[{'yaml_filename': os.path.join(pkg_amr, 'config', 'coe_final.yaml')}],
    )
